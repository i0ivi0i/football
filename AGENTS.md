...
3. **强制全局开启 Graphify 暴力超强模式（--mode deep）**：所有建图、提取与增量更新必须全局强制附带 `--mode deep` 并调用旗舰模型 `gemini-3.8-flash-high`，彻底拒绝浅层扫描与孤立断裂，确保跨文献与盘口之间的微观隐性连线（INFERRED edges）100% 深度挖掘与贯通一体化。赛后必须通过 `graphify save-result` 与 `graphify reflect` 将实际赛果沉淀入知识库，形成复盘进化闭环。
