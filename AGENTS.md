...
1. **赛前查脑与必读总复盘**：**分析推演前必须强制看一下 [总复盘总结 (分析复盘记录/总复盘总结.md)](./分析复盘记录/总复盘总结.md)！** 严格对齐历史全部沉淀教训（伪降水陷阱、极热假深盘、体彩关闭胜平负内幕等），并执行 `graphify query` 检索历史类似盘口与操盘手法（实防、诱盘、阻盘），坚决防止发生任何漏网之鱼与重复失误。
