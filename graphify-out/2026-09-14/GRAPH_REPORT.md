...
> 本次提取的实际 token 用量未报告；下方 0 为占位，不能据此认定没有成本。

## Corpus Check
- 本次仅迁移合并文档的来源并提取防僵化约定；历史节点与待验证状态保留，关联不等于有效性证明。未重新提取原论文。

## Summary
- 522 nodes · 876 edges · 41 communities (25 shown, 16 thin omitted)
- Extraction: 77% EXTRACTED · 8% INFERRED · 14% AMBIGUOUS · INFERRED: 74 edges (avg confidence: 0.67)
...
- 现行规程、智慧复盘与文献入口
- **RESULTS**
- 历史待核查：2026-09-12（周六）中国体彩平局全要素深度复盘与盲区审计报告
- **DISCUSSION**
- **METHOD**
...
4. `微观球员适配器` - 26 edges
5. `本地账本仓储` - 25 edges
6. `PlayeRank Framework` - 21 edges
7. `体彩官方适配器` - 20 edges
8. `Pérez-Blanco & Salmerón (2025) Bayesian Classifier Study` - 20 edges
9. `Sentiment Bias in Betting Odds` - 19 edges
10. `赔率账本契约` - 18 edges

## Surprising Connections (you probably didn't know these)
- `赔率快照` --legacy_unverified_relation--> `Numerical Variant of Shin Conversion`  [AMBIGUOUS]
  脚本/领域/模型.py → 温故而知新学习资料/2604.17194_Forecast_Sports_Outcomes_under_EMH_Odds_Only_Models.md
- `体彩官方适配器` --legacy_unverified_relation--> `Asian Handicap (AH) Betting Market`  [AMBIGUOUS]
  脚本/适配器/体彩接口.py → 温故而知新学习资料/2003.09384_Asian_Handicap_Market_Efficiency_Bayesian_Networks.md
- `体彩官方适配器` --legacy_unverified_relation--> `Constantinou Hybrid Bayesian Network Model`  [AMBIGUOUS]
  脚本/适配器/体彩接口.py → 温故而知新学习资料/2003.09384_Asian_Handicap_Market_Efficiency_Bayesian_Networks.md
...
- `体彩官方适配器` --legacy_unverified_relation--> `历史待核查：2026-09-12 Pre-Match Forecast`  [AMBIGUOUS]
...
### Community 0 - "赔率快照"
Cohesion: 0.05
Nodes (56): 回溯连续轨迹用例, 探测并记录心电图用例, 记录结果, 轨迹结果, 内存模拟提供者, 内存模拟账本, 体彩官方适配器, 本地账本仓储 (+48 more)

### Community 2 - "微观球员适配器"
Cohesion: 0.07
Nodes (32): 微观球员适配器, test_微观球员适配器严格断言完场比分(), test_微观球员适配器提取交锋历史H2H(), test_微观球员适配器提取比赛微观高阶数据(), test_微观球员适配器提取球员高阶链条数据(), test_微观球员适配器解析伤停数据(), test_微观球员适配器计算体能负荷(), test_不同市场与不同比赛禁止计算虚假位移() (+24 more)

### Community 13 - "刷新总对账看板()"
Cohesion: 0.33
Nodes (8): 刷新总对账看板(), 计数(), 读取卡片(), test_对账允许零胜率且不猜头号推荐(), test_空目录不除零(), test_错误计数不覆盖原看板(), write_card(), 汇总复盘卡片的申报计数；不认证赛前封存、不修改经验或训练模型。

### Community 7 - "足球概率预测评分比较（Wheatcroft，2019预印本）"
Cohesion: 0.11
Nodes (18): Wheatcroft 论文 Markdown（附公式与图表）, Wilkens 论文 Markdown（附公式与图表）, Ignorance 对数评分, 概率预测评分, RPS 排序距离假设, 足球概率预测评分比较（Wheatcroft，2019预印本）, 基准与指标适配, 预测评估信息泄漏 (+10 more)

### Community 1 - "现行规程、智慧复盘与文献入口"
Cohesion: 0.05
Nodes (48): 已取代：整改依据与验证边界, 已取代：项目规则与数据口径审计, 已取代：智慧复盘论文索引, 已取代：保存与完整性, 已取代：原文与来源, 已取代：智慧复盘论文索引, 已取代：第二批：条件适应与连续评估, 已取代：面向本项目的应用建议（综合提炼，尚未验证改进效果） (+40 more)
...
### Community 11 - "历史待核查：2026-09-12（周六）中国体彩平局全要素深度复盘与盲区审计报告"
Cohesion: 0.15
Nodes (13): 历史待核查：2026-09-12_复盘.md, 历史待核查：1. 红线 2（伪降水死锁）机械误杀三大豪门客平局（018 摩纳哥 1:1、020 米兰 2:2、028 里昂 0:0）, 历史待核查：1. 赛前已知特征与当时推演假设, 历史待核查：1. 赛前推演与当时假设, 历史待核查：2026-09-12（周六）中国体彩平局全要素深度复盘与盲区审计报告, 历史待核查：2. 真实赛况与血淋淋认知盲区, 历史待核查：2. 真实赛果与 `football-match-analysis` 硬核量化复盘, 历史待核查：2. 红线 4（均势大球前置锁）机械误杀两场对攻大球平局（013 伯恩茅斯 2:2、023 科隆 1:1） (+5 more)
...
### Community 17 - "Q: 体彩官方数据管道与API-Sports微观数据管道在赛前推演中的协同机制"
Cohesion: 0.40
...
### Community 19 - "历史待核查：2026-09-10（周四）中国体彩平局赛前精算推演报告（3.1 硬红线全量过筛版）"
Cohesion: 0.40
Nodes (5): 历史待核查：2026-09-10_预测.md, 历史待核查：🥇 1. 【周四007 解放者杯】德尔瓦耶独立 vs 弗拉门戈（今日唯一黄金王牌 · 评分 94.0）, 历史待核查：2026-09-10（周四）中国体彩平局赛前精算推演报告（3.1 硬红线全量过筛版）, 历史待核查：一、 今日黄金猎物精选榜单, 历史待核查：二、 今日一票否决淘汰场次（全部触犯 3.1 版硬红线）

### Community 20 - "**ABSTRACT**"
Cohesion: 0.40
...
### Community 3 - "Overall Accuracy & Real-time Scoreboard"
Cohesion: 0.06
Nodes (41): 历史待核查：2026-09-09_复盘.md, 历史待核查：1. 🎯 【周三001 江原FC 1:1 全北现代】（赛果：平局 1:1 · 体彩平赔 2.92）, 历史待核查：2026-09-09（周三）中国体彩平局全要素深度复盘报告, 历史待核查：2. 🎯 【周三014 拉普拉塔大学 1:1 科林蒂安】（赛果：平局 1:1 · 体彩平赔 2.58）, 历史待核查：一、 两场命中平局全景精算复盘, 历史待核查：二、 五道硬红线 3.1 版排雷审计（9场分胜负全量排雷成功）, 历史待核查：2026-09-10_复盘.md, 历史待核查：1. 赛前（T-2h）已知客观数据流水 (+33 more)
...
### Community 8 - "历史待核查：2026-09-13_预测.md"
Cohesion: 0.12
Nodes (17): 历史待核查：2026-09-13_预测.md, 历史待核查：1. Capa 1 [ODDS] 市场底牌与心电图轨迹, 历史待核查：1. Capa 1 [ODDS] 盘口结构, 历史待核查：1. Capa 1 [ODDS] 盘口结构, 历史待核查：1. 乔布斯灵活思维解构（破除红线2死板一刀切）, 历史待核查：1. 盘口与战术特征, 历史待核查：2026-09-13（周日）中国体育彩票精算推演报告, 历史待核查：2. Capa 3 [API] 微观防守护城河核验 (+9 more)

### Community 9 - "2023_Aiyer_结果偏见与决策评价.md"
Cohesion: 0.13
...
### Community 5 - "PlayeRank Framework"
Cohesion: 0.13
Nodes (34): Constantinou Hybrid Bayesian Network Model, Modified Pi-Rating System, Deep Neural Network (DNN) Architecture, Match Outcome Classification via Thresholds, Success Score Metric, Tactical Play Styles Framework, Shin Odds Normalization Procedure, Multinomial Logit Regression (MLR) for Soccer (+26 more)

### Community 6 - "Sentiment Bias in Betting Odds"
Cohesion: 0.09
Nodes (32): Bettor State-Space Model (SSM), Bookmaker Implied Probability Regression Models, Live Betting State-Space Model for Fraud Detection, Numerical Variant of Shin Conversion, KTH 2024 Betting Exchange Liquidity Study, Goto et al. (2026) Odds-Only and FL-GLM Conversion Study, Beating the Bookies with Their Own Numbers (Kaunitz et al. 2017), Power Conversion Method (+24 more)

### Community 15 - "项目规则与数据口径审计"
Cohesion: 0.48
Nodes (7): 比例归一化市场概率, Shin逆变换的平方与总和项, 项目规则与数据口径审计, 文献与项目方法笔记资料库, Shin与比例归一化：方法说明及适用边界, 资料关联不等于因果或效果认证, Shin潜在参数的解释边界
...
- `历史待核查：2026-09-12_复盘.md` → `Sentiment Bias in Betting Odds`  [AMBIGUOUS]
  分析复盘记录/2026-09-12_复盘.md · relation: legacy_unverified_relation
- `历史待核查：2026-09-12_复盘.md` → `Overall Accuracy & Real-time Scoreboard`  [AMBIGUOUS]
  分析复盘记录/2026-09-12_复盘.md · relation: legacy_unverified_relation
- `历史待核查：一、 头号猎物【周六017 奥萨苏纳 0:2 西班牙人】崩盘根因深度剖析` → `PlayeRank Framework`  [AMBIGUOUS]
  分析复盘记录/2026-09-12_复盘.md · relation: legacy_unverified_relation
- `历史待核查：一、 头号猎物【周六017 奥萨苏纳 0:2 西班牙人】崩盘根因深度剖析` → `Sentiment Bias in Betting Odds`  [AMBIGUOUS]
  分析复盘记录/2026-09-12_复盘.md · relation: legacy_unverified_relation
...
- `历史待核查：2026-09-11_预测.md` → `历史待核查：2026-09-11 Post-Match Review`  [AMBIGUOUS]
  分析复盘记录/2026-09-11_复盘.md · relation: legacy_unverified_relation
- `历史待核查：2026-09-13_预测.md` → `ELO-Odds Rating System`  [AMBIGUOUS]
  分析复盘记录/2026-09-13_预测.md · relation: legacy_unverified_relation
- `历史待核查：2026-09-13_预测.md` → `PlayeRank Framework`  [AMBIGUOUS]
  分析复盘记录/2026-09-13_预测.md · relation: legacy_unverified_relation
- `历史待核查：2026-09-13_预测.md` → `Hierarchical Bayesian Poisson Football Score Model`  [AMBIGUOUS]
  分析复盘记录/2026-09-13_预测.md · relation: legacy_unverified_relation
- `历史待核查：2026-09-13_预测.md` → `Pérez-Blanco & Salmerón (2025) Bayesian Classifier Study`  [AMBIGUOUS]
  分析复盘记录/2026-09-13_预测.md · relation: legacy_unverified_relation
- `历史待核查：2026-09-13_预测.md` → `Mandadapu (2024) Football Match Outcome Forecasting Study`  [AMBIGUOUS]
  分析复盘记录/2026-09-13_预测.md · relation: legacy_unverified_relation
- `历史待核查：2026-09-13_预测.md` → `Odds-Only Equal Profitability Confidence (OO-EPC)`  [AMBIGUOUS]
  分析复盘记录/2026-09-13_预测.md · relation: legacy_unverified_relation
- `历史待核查：2026-09-13_预测.md` → `Asian Handicap (AH) Betting Market`  [AMBIGUOUS]
  分析复盘记录/2026-09-13_预测.md · relation: legacy_unverified_relation
...
## Knowledge Gaps
- **209 isolated node(s):** `已取代：保存与完整性`, `已取代：原文与来源`, `已取代：第二批：条件适应与连续评估`, `已取代：面向本项目的应用建议（综合提炼，尚未验证改进效果）`, `PDF 第 1 页` (+204 more)
...
- **What is the exact relationship between `探测并记录心电图用例` and `体彩官方适配器`?**
  _Edge tagged AMBIGUOUS (relation: legacy_unverified_relation) - confidence is low._
- **What is the exact relationship between `体彩官方适配器` and `Asian Handicap (AH) Betting Market`?**
  _Edge tagged AMBIGUOUS (relation: legacy_unverified_relation) - confidence is low._
- **What is the exact relationship between `体彩官方适配器` and `Constantinou Hybrid Bayesian Network Model`?**
  _Edge tagged AMBIGUOUS (relation: legacy_unverified_relation) - confidence is low._
- **What is the exact relationship between `体彩官方适配器` and `Bookmakers' Consensus Probability`?**
  _Edge tagged AMBIGUOUS (relation: legacy_unverified_relation) - confidence is low._
- **What is the exact relationship between `体彩官方适配器` and `历史待核查：2026-09-12 Pre-Match Forecast`?**
  _Edge tagged AMBIGUOUS (relation: legacy_unverified_relation) - confidence is low._
- **What is the exact relationship between `本地账本仓储` and `Betting Exchange Liquidity Dynamics`?**
  _Edge tagged AMBIGUOUS (relation: legacy_unverified_relation) - confidence is low._