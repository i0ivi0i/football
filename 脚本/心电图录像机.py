...
def 执行单次探测(记录用例: 探测并记录心电图用例):
    ...
    if 结果.关闭胜平负场次:
        print("\n🔒 【捕获体彩官方故意关闭基础胜平负场次 (红线7·明牌开卷考绝不跳过)】:")
        for 关盘 in 结果.关闭胜平负场次:
            print(f"  🔥 {关盘.竞彩场次:<7} | {关盘.联赛名称:<6} | {关盘.对阵名称} | 让球赔率: [{关盘.赔率.主胜:.2f}, {关盘.赔率.平局:.2f}, {关盘.赔率.客胜:.2f}] (必须强制推演，严禁跳过！)")
    if 结果.异常异动警报:
        print("\n🚨 【捕获显著倍率异常异动警报】:")
        for 警报 in 结果.异常异动警报:
            print(f"  ⚡ {警报.异动信号}: 平项赔率变动 {警报.平赔变动值:+.2f} | 变盘速度 {警报.每小时变盘速率:+.2f}/小时")
    else:
...
def 展示比赛轨迹(分析用例: 回溯连续轨迹用例, 竞彩场次: str):
    ...
    for 序号, 快照 in enumerate(结果.历史快照列表, 1):
        ...
        print(f"  #{序号} [{时间字串}] 胜:{快照.赔率.主胜:4.2f} 平:{快照.赔率.平局:4.2f} 负:{快照.赔率.客胜:4.2f} | 市场平项隐含概率:{平率*100:5.2f}% | 变动:{标记符号}")
